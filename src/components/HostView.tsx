...
export const HostView: React.FC = () => {
  ...
  // Quick join QR code states
  const [isQrZoomed, setIsQrZoomed] = useState(false);
  const [joinUrl, setJoinUrl] = useState("");

  useEffect(() => {
    if (typeof window !== "undefined") {
      setJoinUrl(`${window.location.origin}/player?pin=${pin}`);
    }
  }, [pin]);
  ...
  const currentChallenge =
    activeStage >= 1 && activeStage <= 10 ? CHALLENGES[activeStage - 1] : null;
  const currentSubmissions = submissions.filter(
    (s) => s.challengeId === activeStage,
  );
  ...
  const renderQrZoomModal = () => {
    if (!isQrZoomed || !joinUrl) return null;
    return (
      <div
        className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-50 flex flex-col items-center justify-center p-6 transition-all duration-300"
        onClick={() => setIsQrZoomed(false)}
      >
        <div
          className="max-w-md w-full bg-zinc-950/80 border border-white/10 rounded-[32px] p-8 shadow-2xl flex flex-col items-center text-center space-y-6 relative animate-[fadeIn_0.3s_ease-out]"
          onClick={(e) => e.stopPropagation()}
        >
          <button
            onClick={() => setIsQrZoomed(false)}
            className="absolute top-5 right-5 text-zinc-400 hover:text-white font-mono text-sm bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-full transition-all"
          >
            ✕ Tutup
          </button>

          <div className="space-y-1">
            <span className="text-[10px] font-mono text-indigo-400 tracking-[0.25em] uppercase">
              PIN PINDAI CEPAT
            </span>
            ...
            <p className="text-xs text-zinc-400 max-w-xs mx-auto">
              Dekatkan kamera HP Anda ke layar proyektor untuk masuk secara
              otomatis tanpa perlu mengetik PIN.
            </p>
          </div>

          <div className="p-6 bg-white rounded-3xl shadow-[0_0_40px_rgba(99,102,241,0.3)] hover:scale-[1.02] transition-transform duration-300">
            <QRCodeSVG
              value={joinUrl}
              size={280}
              level="H"
          ...
          <div className="space-y-2 w-full bg-[#14141E] border border-white/5 rounded-2xl p-4">
            <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest">
              KODE KELAS
            </span>
            <div className="text-4xl font-mono tracking-widest font-extrabold text-indigo-400 select-all">
              {pin}
            </div>
            <div className="text-[10px] text-zinc-500 font-mono truncate max-w-xs mx-auto mt-1">
              {joinUrl}
            </div>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-[#0D0D11] text-zinc-100 flex flex-col justify-center items-center p-8 relative overflow-hidden">
        ...
        <div className="max-w-md w-full backdrop-blur-md bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl space-y-6 text-center z-15">
          ...
          <p className="text-xs font-mono text-zinc-400 tracking-wider">
            HIMA TRPL BONDING HOST CONSOLE
          </p>

          <div className="p-5 bg-[#14141E]/80 border border-white/5 rounded-2xl flex flex-col items-center gap-3">
            <span className="text-[10px] font-mono tracking-widest text-zinc-500 uppercase">
              ROOM PIN GENERATED
            </span>
            <p className="text-5xl font-mono tracking-wider font-extrabold text-indigo-400 mt-2">
              {pin}
            </p>
            {joinUrl && (
              <div
                onClick={() => setIsQrZoomed(true)}
                className="mt-2 p-2.5 bg-white rounded-xl cursor-pointer hover:scale-105 transition-transform relative group shadow-lg"
                title="Klik untuk memperbesar QR"
              >
                <QRCodeSVG value={joinUrl} size={110} level="M" />
                <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-xl transition-opacity text-[10px] text-white font-mono font-bold">
                  🔍 ZOOM
                </div>
              </div>
            )}
          ...
          <div className="text-zinc-400 text-xs leading-relaxed max-w-xs mx-auto">
            Gunakan PIN ini untuk login oleh semua peserta di HP mereka.
            Pastikan server lokal websocket sudah berjalan.
          </div>
        ...
        <div className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 backdrop-blur-md bg-white/5 border border-white/10 rounded-3xl p-6 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <ul className="space-y-3 text-xs text-zinc-400 leading-relaxed font-sans list-decimal list-inside">
                <li>
                  Minta seluruh mahasiswa HIMA TRPL membuka web di handphone
                  masing-masing.
                </li>
                <li>
                  Atau scan QR Code di bawah untuk bergabung langsung tanpa
                  perlu mengetik PIN.
                </li>
                <li>
                  Gunakan nama panggilan asli agar fotonya gampang dikenali.
                </li>
                <li>
                  Setelah semua peserta terhubung, tekan tombol{" "}
                  <b>"Mulai Sesi Bonding"</b>.
                </li>
              </ul>

              {joinUrl && (
                <div className="p-4 bg-[#14141E]/80 border border-white/5 rounded-2xl flex flex-col items-center text-center space-y-2 relative group mt-4">
                  <span className="text-[9px] font-mono tracking-widest text-indigo-400 uppercase">
                    PINTASAN SCAN QR
                  </span>
                  <div
                    onClick={() => setIsQrZoomed(true)}
                    className="p-2.5 bg-white rounded-xl cursor-pointer hover:scale-105 transition-transform shadow-lg relative"
                    title="Klik untuk memperbesar QR"
                  >
                    <QRCodeSVG value={joinUrl} size={110} level="M" />
                    <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-xl transition-opacity text-[10px] text-white font-mono font-bold">
                  ...
                  <div className="text-[9px] text-zinc-500 font-mono select-all">
          ...
          <div className="lg:col-span-2 backdrop-blur-md bg-white/5 border border-white/10 rounded-3xl p-6 flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-base font-mono tracking-wider text-zinc-300 uppercase">
                Peserta Terkoneksi ({players.length})
              </h2>
  ...
  if (activeStage >= 1 && activeStage <= 10) {
    return (
      <div className="min-h-screen bg-[#0A0A0F] text-zinc-100 flex flex-col p-8 font-sans relative overflow-hidden">
        ...
        <div className="flex justify-between items-center pb-6 border-b border-white/5 mb-8">
          <div>
            <span className="text-[9px] font-mono text-rose-300 tracking-widest uppercase">
              SESI BONDING TERJALIN
            </span>
          ...
          <div className="flex items-center gap-3">
            {countdown !== null ? (
              ...
              <div className="px-4 py-2 bg-zinc-900 border border-white/10 rounded-xl text-xs font-mono text-zinc-400">
                Submitted: {currentSubmissions.length} / {players.length}{" "}
                Players
              </div>
            )}
            <button
        ...
        <div className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 backdrop-blur-md bg-white/5 border border-white/10 rounded-3xl p-6 space-y-6 flex flex-col justify-between">
            ...
            <div className="text-zinc-500 text-[10px] font-mono leading-normal pt-4 border-t border-white/5">
              Tekan tombol "Lanjut" di kanan atas jika waktu pengerjaan dirasa
              cukup, atau semua peserta telah mengirim jawabannya.
            </div>
          </div>
          ...
          <div className="lg:col-span-2 backdrop-blur-md bg-white/5 border border-white/10 rounded-3xl p-6 flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-sm font-mono tracking-widest text-zinc-300 uppercase">
                SUBMISSIONS FEED ({currentSubmissions.length})
              </h2>
              <span className="text-xs text-zinc-500 font-serif italic">
                Terisi secara real-time dari HP peserta
              </span>
            </div>
            ...
            {currentSubmissions.length === 0 ? (
              <div className="flex-grow flex flex-col items-center justify-center text-center p-8 border border-dashed border-white/5 rounded-2xl">
                ...
                <p className="text-xs font-mono text-zinc-500 tracking-wider">
                  MENUNGGU SUBMISSI DARI PESERTA...
                </p>
              </div>
            ) : (
                    ...
                    selectedSub.detectedPlayers.length > 0 && (
                      <div>
                        <span className="text-[10px] font-mono text-emerald-400 tracking-wider">
                          AI FACE DETECTION MATCH (
                          {selectedSub.detectedPlayers.length})
                        </span>
                        <div className="flex flex-wrap gap-1.5 mt-1.5">
                          {selectedSub.detectedPlayers.map((pn, pidx) => (
                            <span
                              key={pidx}
                              className="px-2 py-0.5 bg-emerald-950/40 border border-emerald-500/30 rounded-lg text-[10px] font-mono text-emerald-300"
                            >
                              ✓ {pn}
                            </span>
                          ))}
    ...
    <div className="min-h-screen bg-[#08080C] text-zinc-100 flex flex-col p-8 font-sans relative overflow-x-hidden">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center pb-6 border-b border-white/5 mb-8 gap-4">
        <div>
          ...
          <h1 className="text-3xl font-serif italic text-white mt-1">
            Melebur Tanpa Jarak ✨
          </h1>
          <p className="text-xs text-zinc-400 mt-1">
            Berikut adalah semua potret konyol dan curahan hati paling jujur
            dari teman-teman HIMA TRPL.
          </p>
        </div>
            ...
            <>
              ...
              >
                <option value="" disabled>
                  🎞️ Unduh Strip Peserta...
                </option>
                {Array.from(new Set(submissions.map((s) => s.playerName))).map(
                  (name, pIdx) => (
                    <option key={pIdx} value={name}>
                      {name}
                    </option>
                  ),
                )}
      ...
      {submissions.length === 0 ? (
        <div className="flex-grow flex flex-col justify-center items-center text-center p-8 border border-dashed border-white/10 rounded-3xl bg-zinc-900/5">
          ...
          <h2 className="text-xl font-serif italic text-zinc-400">
            Belum ada foto yang terkumpul.
          </h2>
          <p className="text-xs text-zinc-600 max-w-sm mt-1">
            Coba jalankan permainan dan submit foto dari handphone untuk melihat
            karya di sini.
          </p>
        </div>
      ) : (
